...
The SLM acts only in the transverse $(x,y)$ plane and performs phase-only modulation. The applied
phase pattern is $\phi(x,y)$, so the output field is
\[
E_{\mathrm{SLM}}(x,y)=E_{\mathrm{in}}(x,y)e^{i\phi(x,y)}.
\]
This phase pattern is the control input to the system.
\newpage
...
\begin{enumerate}
    \item[(I)] Define the incident field $E_{\mathrm{in}}(x,y)$ at the SLM plane.
    \item[(II)] Apply SLM phase modulation to obtain $E_{\mathrm{SLM}}(x,y)=E_{\mathrm{in}}(x,y)e^{i\phi(x,y)}$.
    \item[(III)] Propagate through the thermo-optical material using the transmission matrix $T_{\mathrm{opt}}(t)$ to give $E_{\mathrm{out}}=T_{\mathrm{opt}}(t)E_{\mathrm{SLM}}$.
    \item[(IV)] Compute the output intensity $I(x,y)=|E_{\mathrm{out}}(x,y)|^2$.
    \item[(V)] Define the reinforcement-learning reward as the intensity at a selected target output location.
\end{enumerate}
