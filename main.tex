...
Minimising \(\mathcal{L}\) means differentiating this scalar objective with respect to the trainable
parameters \(\theta = \{W^{[1]}, b^{[1]}, \dots, W^{[N]}, b^{[N]}\}\) and updating the weights and biases
iteratively (for instance by gradient descent). The intermediate quantities \(z^{[\ell]}\) and
\(a^{[\ell]}\) arise during each forward pass and are not directly updated; instead, changes in
\(W^{[\ell]}\) and \(b^{[\ell]}\) produce new forward-pass values and thus a new temperature prediction
\(T_\theta(\mathbf{r},t)\).
...
directs the optimiser to prioritise satisfying that constraint more strongly.
...
\]
The intermediate values $z^{[\ell]}$ and $a^{[\ell]}$ are produced during each forward pass and are
not directly updated by the loss; instead, changing $W^{[\ell]}$ and $b^{[\ell]}$ yields a new forward
pass and therefore a new prediction $T_\theta(x,t)$. This is the mechanism by which the PDE,
initial, and boundary constraints influence the learned solution.
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item Form the PDE residual $R_\theta(x,t)$ by substituting these derivatives into
        the heat equation.
    \item Repeat steps (i)--(v) over many sampled points and evaluate the total loss
        $\mathcal{L}$ over all collocation, initial, and boundary sets.
    \item Update $W^{[\ell]}$ and $b^{[\ell]}$ using gradients of $\mathcal{L}$, and iterate
        until convergence.
\end{enumerate}
